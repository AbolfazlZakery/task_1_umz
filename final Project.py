...
def buy_ticket_button_clicked(buyer, email):
    ...
    if not bool(re.match(email_pattern, email)):
        ...
        if send_email(email, "Event information", message):
            ...
            send_email(admin_g, "Task your event",
                       f"A person named {buyer} bought your event\nPlease do the work for this buyer and manage the event for this person\n\n\nEvent management system")
...
def open_registration_window():
    ...
    save_button = tk.Button(registration_window, text="Save",
                            command=lambda: save_to_database(username_entry.get(), password_entry.get(),
                                                             email_entry.get()))
    save_button.pack()
...
def sign_inup(name_event):
    def callback():
        ...
        buy_ticket_bitton = tk.Button(popup_window2, text="Sign in & buy ticket",
                                      command=lambda: check_info(user_entry.get(), pass_entry.get()))
...
def on_event_select(event):
    ...
    details_label = tk.Label(popup_window,
                             text=f"Date: {selected_event.date}\nTime: {selected_event.time}\nLocation: {selected_event.location}\nDescription: {selected_event.description}")
    details_label.pack()

    buy_ticket_button = tk.Button(popup_window, text="Buy ticket", command=sign_inup(selected_event.name))
    buy_ticket_button.pack()

    show_participants_button = tk.Button(popup_window, text="Show Participants",
                                         command=lambda: show_participants(selected_event))
...
background_image = PhotoImage(
    file=r"C:\Users\yasin\Downloads\11zon_converted\time___the_world_clock___worldwide-wallpaper-2880x1800_2_11zon.png")
